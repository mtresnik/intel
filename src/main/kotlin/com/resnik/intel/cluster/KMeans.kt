...
class KMeans(val seedPoints: Collection<ArrayPoint>) : Comparable<KMeans> {
    ...
    fun cluster(points: Collection<ArrayPoint>): List<Cluster> {
        ...
        points.forEach { point ->
            val index = getClusterIndex(point); allClusters[index].add(point); clusterIndices.add(
            index
        )
        }
    ...
    fun getClusterIndex(point: ArrayPoint): Int =
        allClusters.indices.minByOrNull { index -> allClusters[index].distanceTo(point) }!!
    ...
    companion object {
        ...
        fun getDataPoints(seedIndices: Set<Int>, data: List<ArrayPoint>) =
            data.indices.takeWhile { it !in seedIndices }.map { data[it] }
        ...
        fun getBestKMeans(size: Int, data: List<ArrayPoint>, iterations: Int = data.size / 2): KMeans {
            ...
            repeat(iterations - 1) {
                ...
                while (seedIndices in seedPool) {
                    seedIndices = randomSeedIndices(size, data.lastIndex)
                }
                val seedPoints = getSeedPoints(seedIndices, data)
                val dataPoints = getDataPoints(seedIndices, data)
                val ret = KMeans(seedPoints)
