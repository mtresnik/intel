...
abstract class CSPBase<VAR, DOMAIN>(
    ...
    fun preprocess() {
        preprocessors.forEach { it.preprocess(this) }; println("Preprocessing complete in: ${preprocessors.sumOf { it.finalTime() }}")
    }

    override fun finalTime(): Long = super.finalTime() + preprocessors.sumOf { it.finalTime() }

    override fun getVariables(): List<VAR> {
        return mapStorage?.getVariables() ?: if (!sortVariables) variables else getSortedVariables(
            variables,
            localConstraints
        )
    }
    ...
    protected fun getDomainMap(
        vars: List<VAR> = getSortedVariables(
            variables,
            localConstraints
        )
    ): Map<VAR, List<DOMAIN>> {
    ...
    protected fun addLocalConstraintInternal(constraint: LocalConstraint<VAR, DOMAIN>) {
        for (variable in constraint.variables) {
            if (variable in this.variables) {
                val constraintLookup: MutableList<LocalConstraint<VAR, DOMAIN>> =
                    this.localConstraints[variable] ?: mutableListOf()
    ...
    protected fun isReusablyConsistent(assignment: Map<VAR, DOMAIN>): Boolean {
        ...
        reusableConstraints.addAll(
            localConstraints.values.flatten().filterIsInstance<ReusableConstraint<VAR, DOMAIN>>()
        )
    ...
    companion object {
        ...
        private fun <VAR, DOMAIN> getSortedVariables(
            variables: List<VAR>,
            constraints: Map<VAR, MutableList<LocalConstraint<VAR, DOMAIN>>>
        ): List<VAR> =
            if (constraints.isEmpty()) variables else variables.sortedBy { constraints[it]?.size ?: 0 }.reversed()

        internal fun <VAR, DOMAIN> convertToDomainMap(
            variables: List<VAR>,
            domains: List<DOMAIN>
        ): Map<VAR, List<DOMAIN>> {
