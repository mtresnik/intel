...
class FeedForwardNeuralNetwork : Model {
    ...
    constructor(vararg layerSizes : Int, biases: ArrayVector = ArrayVector(layerSizes.size, 0.0), transferFunction: TransferFunction = sigmoid) {
        ...
        this.weights = Array(this.layers.size - 1){ ArrayMatrix(layers[it + 1].size(), layers[it].size()) { Math.random() } }
