...
class LSTM(inputSize: Int, val outputSize: Int, val learningRate: Double) {
    ...
    fun forward() {
        forget =
            (this.forgetState.transpose() * this.inputLayer.toColMatrix()).apply { sigmoid.activate(it) }.toVector()!!
        ...
        output =
            (this.outputGate.transpose() * this.inputLayer.toColMatrix()).apply { sigmoid.activate(it) }.toVector()!!
        this.outputLayer = output.hadamard(cellState.apply { tanh.activate(it) })
    }

    fun backward(
        error: ArrayVector,
        allCellStates: ArrayVector,
        forget: ArrayVector,
        input: ArrayVector,
        cell: ArrayVector,
        output: ArrayVector,
        cellDerivative: ArrayVector,
        hiddenStateDerivative: ArrayVector
    ) {
        ...
        outputUpdate = outputDerivative.hadamard(output.apply { tanh.derivative(it) })
            .toColMatrix() * this.inputLayer.toColMatrix().transpose()
        val derivativeCellState = (updatedError.hadamard(output)
            .hadamard(cellState.apply { tanh.derivative(it) } + cellDerivative)).apply {
            it.coerceIn(
                MIN_WEIGHT,
                MAX_WEIGHT
            )
        ...
        cellUpdate =
            derivativeCell.hadamard(cell.apply { tanh.derivative(it) }).toColMatrix() * this.inputLayer.toColMatrix()
                .transpose()
        val inputDerivative = derivativeCellState.hadamard(cell)
        inputUpdate = inputDerivative.hadamard(input.apply { sigmoid.derivative(it) })
            .toColMatrix() * this.inputLayer.toColMatrix().transpose()
        val derivativeForget = derivativeCellState.hadamard(allCellStates)
        forgetUpdate = derivativeForget.hadamard(forget.apply { sigmoid.derivative(it) })
            .toColMatrix() * this.inputLayer.toColMatrix().transpose()
    ...
    fun update(
        forgetUpdate: ArrayMatrix,
        inputUpdate: ArrayMatrix,
        cellUpdate: ArrayMatrix,
        outputUpdate: ArrayMatrix
    ) {
        ...
        this.forgetState -= this.forgetGateGradient.apply { this.learningRate / kotlin.math.sqrt(it + NON_ZERO) }
            .hadamard(forgetUpdate)
        this.inputGate -= this.inputGateGradient.apply { this.learningRate / kotlin.math.sqrt(it + NON_ZERO) }
            .hadamard(inputUpdate)
        this.cellStateMat -= this.cellStateGradient.apply { this.learningRate / kotlin.math.sqrt(it + NON_ZERO) }
            .hadamard(cellUpdate)
        this.outputGate -= this.outputGateGradient.apply { this.learningRate / kotlin.math.sqrt(it + NON_ZERO) }
            .hadamard(outputUpdate)
    }
