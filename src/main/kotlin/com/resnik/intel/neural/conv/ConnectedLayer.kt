...
class ConnectedLayer(val outputLength: Int, val inputDimSizes: IntArray) : TensorLayer {
    ...
    var weights: ArrayTensor =
        ArrayTensor(weightDims) { RANDOM.nextGaussian() * sqrt(2.0) / sqrt(inputDimSizes.product().toDouble()) }
    ...
    override fun backward(outputGrads: ArrayTensor, learningRate: Double): ArrayTensor {
        ...
        outputGrads.values.indices.forEach {
            ...
            weightGradient =
                if (weightGradient == null) newGrads else weightGradient!!.append(newGrads, weights.dim.size())
        }
