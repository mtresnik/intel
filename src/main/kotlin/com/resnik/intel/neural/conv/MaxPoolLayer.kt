...
class MaxPoolLayer(strides : IntArray, sizes : IntArray, val expectedInputDims : IntArray) : TensorLayer {
    ...
    val outputDims : IntArray = IntArray(expectedInputDims.size){ (ceil((expectedInputDims[it] - this.sizes[it] + 1).toDouble() / this.strides[it])).toInt()}
    ...
    override fun backward(outputGrads: ArrayTensor, learningRate: Double): ArrayTensor {
        ...
        while(regionIterator.hasNext()) {
            ...
            val maxCoords = ArrayTensorIndex.fromInt(maxIndex,
                ArrayDimension(*sizes)
            )
            inputGrads[ArrayTensorIndex(*(regionIterator.coords() + maxCoords.values))] = outputGrads.values[regionIterator.numTraversed() - 1]
        }
        return inputGrads
    }
